#!/bin/bash
# Off-host backup of literature.learnche.org to AWS S3.
#
...
#        s3://$BACKUP_S3_BUCKET/$BACKUP_S3_PREFIX/db/daily/db_literature-YYYY-MM-DD.sql.gz
#      On the 1st of each month the same dump is also copied to
#        .../db/monthly/db_literature-YYYY-MM.sql.gz
#      and on Jan 1 to
#        .../db/yearly/db_literature-YYYY.sql.gz
#
#   2. `aws s3 sync` of the bind-mounted dirs (data/media/ and
#      data/public/) to the matching S3 prefixes. data/media/ holds the
#      admin-uploaded PDFs (the FTS source) — irreplaceable. data/public/
#      holds the small files Caddy aliases (robots.txt, favicon.ico).
#      No --delete: an accidental local rm or detached bind-mount must NOT
#      propagate to the off-host copy. data/static/ is intentionally
#      skipped — collectstatic regenerates it on every container start.
#
...
#   BACKUP_S3_BUCKET                  e.g. openmv-backups (shared with openmv)
#   BACKUP_S3_PREFIX                  optional, defaults to "literature"
...
#
# 22:35 is offset by one hour from openmv's 21:35 so the two stacks on the
# same Hetzner box don't fight for upload bandwidth.
...
BACKUP_S3_PREFIX="${BACKUP_S3_PREFIX:-literature}"
...
DB_FILE="$TMP_DIR/db_literature-${DATE_DAILY}.sql.gz"

log() {
    echo "[backup-literature] $(date -u +%FT%TZ) $*"
}
...
if [[ "$DAY_OF_MONTH" == "01" ]]; then
    aws s3 cp \
        ...
        "${S3_BASE}/db/monthly/db_literature-${DATE_MONTH}.sql.gz"
    log "promoted to monthly: db_literature-${DATE_MONTH}.sql.gz"
fi

if [[ "$MONTH_OF_YEAR" == "01" && "$DAY_OF_MONTH" == "01" ]]; then
    aws s3 cp \
        "${S3_BASE}/db/daily/$(basename "$DB_FILE")" \
        "${S3_BASE}/db/yearly/db_literature-${DATE_YEAR}.sql.gz"
    log "promoted to yearly: db_literature-${DATE_YEAR}.sql.gz"
fi

# 3. Mirror the PDF library + small public files. No --delete (see header).
