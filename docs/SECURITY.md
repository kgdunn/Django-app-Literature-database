...
| 5  | Medium   | No `robots.txt`                           | `/admin/` was indexable by search engines. `test.literature.learnche.org` had no `noindex`.                                                                                                                                                                | **Fixed (#72).** `pages.robots_txt` serves `/robots.txt` from Django (`Disallow: /admin/`, `/accounts/`, `/__extract_extra__/`); staging emits `X-Robots-Tag: noindex, nofollow` via `SecurityHeadersMiddleware` when `LITERATURE_NOINDEX` is set. Covered by `TestStagingNoindex` + the robots.txt test.                                |
| 6  | Medium   | `literature/middleware.py` CSP            | `script-src` allowed `https://cdn.jsdelivr.net` for MathJax + ECharts. SRI hashes were pinned, but a tightened CSP would drop the CDN allowance entirely.                                                                                                    | **Fixed in v1.5.0 (#79).** Both libraries vendored under `literature/static/literature/vendor/` and served from `/static`; `script-src` is now a bare `'self'`. MathJax bumped 2.7.9 → 3.2.2 **SVG** bundle (single self-contained file — v2 can't be vendored without ~63 MB of lazily-loaded fonts). SVG output fetches no web fonts, so no `font-src` widening for math. SRI dropped as redundant for same-origin assets; provenance + SHA-256s recorded in the vendor `README.md`. |
...
| 8  | Low      | `Dockerfile:38`                           | Runtime apt installs `libpq5` for `psycopg2-binary`. Switching to psycopg3 would drop the apt-install line entirely.                                                                                                                                       | **Fixed in v1.0.1 (PR #97).** `pyproject.toml` now pins `psycopg[binary]>=3.1,<4`; `psycopg[binary]` ships its own bundled libpq, so the runtime stage no longer apt-installs `libpq5`. Closes #85. Removes the pre-req blocker for Phase 12 (pgvector, #64).                                                                            |
...
- ~~**#72**~~ — `robots.txt` + staging `noindex`. **Closed (completed).**
...
- ~~**#79**~~ — Vendor MathJax + ECharts; drop CDN from CSP. **Closed in v1.5.0.** MathJax bumped to the v3.2.2 SVG bundle (single self-contained file); `script-src` is now `'self'`.
...
- ~~**#85**~~ — Migrate `psycopg2-binary` → psycopg3. **Closed in v1.0.1 (PR #97).** Pre-req for Phase 12 (pgvector, #64) cleared.
