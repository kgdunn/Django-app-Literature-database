...
Docstring corrections and one surgical bug fix, so the documentation
...
- **`items/models.py`** — **CODE FIX** — `ConferenceProceeding.full_citation`
  ...
  proceeding with a publisher set raised ``TypeError: sequence item N:
  expected str instance, Publisher found`` at render time. Coerced
...
- **`pagehit/views.py`** — `create_hit`'s docstring said "If the
  ...
  the row is still written, with ``item_pk=0``, which quietly
