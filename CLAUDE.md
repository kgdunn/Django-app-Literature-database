...
- **Models**:
  - `items.Item` — abstract-ish base. Multi-table inheritance: subclasses `JournalPub`, `Book`, `ConferenceProceeding`, `Thesis` each create their own DB table joined to `Item` via an implicit OneToOne. Item fields: `title`, `slug`, `item_type` (`thesis|journalpub|book|conferenceproc`), `year`, `doi_link`, `web_link`, `abstract`, `date_created` (`auto_now=True`), `pdf_file` (`upload_to=literature/pdf/{slug[0]}/{slug}.pdf`; **admin-only by default**), `pdf_is_public` (BooleanField, `default=False` — the per-item override added in v1.4.0 that exposes `pdf_file` to visitors via the gated `view_pdf` view; off for every copyright-restricted PDF), `other_search_text` (free-text search bucket — auto-extracted from the PDF or hand-curated). Phase 5 dropped the legacy `private_pdf` and `can_show_pdf` flags along with the unconditional public `download_item` view (copyright restriction; PDFs are admin-internal unless `pdf_is_public` is ticked, and otherwise consumed only by `__extract_extra__` for FTS extraction). v1.1.0 dropped the legacy `show_abstract` boolean — abstracts now render on the detail page whenever the field is non-empty, no per-item opt-in gate. Custom manager `LatestItemManager` orders by `-date_created`.
...
- **Views** (`items/views.py`, `pages/views.py`):
  ...
  - `pages.search` — `/search?q=<terms>` — search results. After Phase 3, this is the canonical Postgres-FTS endpoint (whitespace-AND tokens, weighted `SearchVector` over title/abstract/other_search_text plus the issue-#33 subclass fields + `TrigramSimilarity` for fuzzy author matching). Pre-Phase 3, this is a Haystack passthrough. Empty / missing `q` returns the front page.
  - `items.show_items` — `/item/show-all`, `/item/<view>/<slug>/`, `/item/pub-by-year/<year>/` — paginated list view, parameterized by `what_view` (`all`, `tag`, `author`, `journal`, `pub-by-year`, `show`). There is no `sort` branch.
  - `items.view_item` — `/item/<id>/<slug>` (slug optional) — detail page. Renders a "View PDF" link only when `item.pdf_is_public` and `item.pdf_file` are both set.
  - `items.view_pdf` — `/item/<id>/pdf` (URL name `lit-public-pdf`) — the gated public-PDF endpoint added in v1.4.0. Streams `Item.pdf_file` inline (`Content-Type: application/pdf`, `as_attachment=False`) **only** when `Item.pdf_is_public=True` and a `pdf_file` is attached; otherwise 404. Default-off, so the catalogue stays default-deny per item; copyright-restricted PDFs simply leave the box unticked. Registered *before* the `lit-view-item` catch-all in `items/urls.py` so `/item/<id>/pdf` doesn't get swallowed as a slug.
  ...
  - **The only public PDF path is the gated `view_pdf` view.** Phase 5 removed the *unconditional* `items.download_item` and the `lit-download-pdf` URL name; v1.4.0 added back a narrow, default-off path via `Item.pdf_is_public` + `view_pdf`. Outside of an explicitly-ticked item, `Item.pdf_file` is admin-only storage consumed only by `__extract_extra__` for FTS extraction. The Caddy `/media/literature/pdf/*` path stays excluded from the static `file_server` rule in production (public PDFs go through `view_pdf`, never the static path) — see "Production deployment" below.
...
- **Templates** (`templates/` + per-app `{app}/templates/{app}/`):
  - `templates/base.html` — site layout, mirroring openmv's design system: design-token CSS (light + dark theme) in `literature/static/literature/site.css`, IBM Plex Sans/Serif/Mono via Google Fonts. Includes the `<meta viewport>` for mobile, a tri-state theme toggle (light / dark / auto, persisted in `localStorage`, driven by the static `theme-preload.js` / `theme-toggle.js`), and — since issue #79 — a **self-hosted MathJax 3.2.2 SVG bundle** (`literature/static/literature/vendor/tex-mml-svg.js`, configured by the static `mathjax-config.js`) for `\(...\)` LaTeX in abstracts. The SVG bundle embeds all glyph paths, so it fetches no web fonts at runtime. ECharts 5.5.1 (the tag/author sparkline) is likewise self-hosted at `vendor/echarts.min.js`. Both were vendored off `cdn.jsdelivr.net` (#79) so the CSP `script-src` is a bare `'self'` — the only remaining off-origin allowances are Google Fonts (`style-src`/`font-src`). Provenance + refresh steps live in `literature/static/literature/vendor/README.md`.
...
1. **Postgres FTS** (Phase 3 — Stage 1, canonical) — `pages.search` builds a weighted `SearchVector` over `Item.title` (A), `Item.abstract` (B), `Item.other_search_text` (C), plus the subclass-specific fields added for issue #33 — `journalpub__journal__name` (B), `book__isbn` (C), `conferenceproceeding__conference_name` (B), `conferenceproceeding__organization` (C), `conferenceproceeding__location` (C), `incollection__book_title` (B) — ranked with `SearchRank` (`cover_density=True`, issue #15), OR-joined with `__trigram_similar` on author last names for fuzzy author matching. Uses `SearchQuery(..., search_type='websearch')` so bare terms AND, quoted phrases preserve order, and `-foo` excludes. Backed by `CREATE EXTENSION IF NOT EXISTS pg_trgm` (`items` migration `0004_pg_trgm`). No GIN index yet — the corpus is small enough that a sequential scan is sub-100 ms; revisit in Phase 9/10 if benchmarks demand it.
...
3. **PDFs are admin-only by default; the only sanctioned public path is the per-item `pdf_is_public` override.** Phase 5 removed the *unconditional* `items.download_item` and the `lit-download-pdf` URL name, plus the legacy `Item.private_pdf` / `Item.can_show_pdf` flags. v1.4.0 reintroduced a narrow, **default-off** override: `Item.pdf_is_public` (BooleanField, `default=False`) + the gated `items.view_pdf` view (`/item/<id>/pdf`, URL name `lit-public-pdf`). `view_pdf` 404s unless the item has `pdf_is_public=True` **and** a `pdf_file`; only then does it stream the bytes inline from the gunicorn worker. **Default-deny is the invariant** — every item stays non-downloadable until an admin ticks the box for a specific, copyright-cleared document. The catalogue still holds copyright-restricted PDFs uploaded for FTS extraction (`__extract_extra__` reads the bytes and writes the text into `Item.other_search_text`); those stay unticked. **Don't bypass the flag:** never add a view/template that returns `Item.pdf_file.read()` or links to `/media/literature/pdf/...` without checking `pdf_is_public` first, never reuse the dead `lit-download-pdf` name, and link to `{% url 'lit-public-pdf' item.pk %}` rather than a raw media URL. In production the Caddy server block in `docs/deploy.md` still 404s `/media/literature/pdf/*` (in front of the generic `/media/*` `file_server`): public PDFs are served *only* through `view_pdf`, never the static path, so the in-DB flag stays the single gate. **Defense-in-depth, not Caddy-only:** the on-disk path is the *guessable* `literature/pdf/<slug[0]>/<slug>.pdf` shape (the slug comes straight off the public title), so a visitor who downloads one *public* PDF could otherwise reconstruct the media URL for a *non-public* item. To stop that in every environment — not just behind Caddy — `literature/urls.py` registers a `_block_media_pdf` view that hard-404s `^media/literature/pdf/` *before* the `DEBUG`-only `static()` media handler, mirroring the Caddy rule inside Django (dev/staging/any future infra change can't leak a PDF either). Net: the gated `view_pdf` is the **only** route to PDF bytes in all environments. Still don't link to `/media/literature/pdf/...` from any template — link to `{% url 'lit-public-pdf' item.pk %}`.
...
- **Vendored front-end assets** (`literature/static/literature/vendor/` — MathJax SVG bundle + ECharts, issue #79) are checked in byte-for-byte from `cdn.jsdelivr.net` and pinned by SHA-256 in that dir's `README.md`, which also records a **`Last vendored:` date**. Unlike the Python deps and GitHub Actions, **Dependabot cannot bump these** — there is no automated update path, so they only move when a human/Claude moves them. **Maintenance reminder for Claude:** on any task that already touches this repo, check the `Last vendored:` date — if it's been **roughly 6 months or more**, refresh the bundles to the current stable upstream versions *as part of that same PR* (follow the refresh steps in the vendor `README.md`: re-download, update the version + SHA-256 + `Last vendored:` date, bump `version`/`RELEASES.md`, and note that a browser smoke-check — math renders, sparkline draws, no CSP violations — is the one thing the automated tests can't verify). Don't open a standalone PR just for this; fold it into whatever change is already in flight. If it's clearly under ~6 months, leave them alone. Stay on the MathJax **SVG** bundle (`tex-mml-svg.js`), not CHTML, so no web fonts get pulled at runtime (that would force a `font-src` CSP widening).
